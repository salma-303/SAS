...
face_cascade = cv2.CascadeClassifier('haarcascade_frontalface_default.xml') #detect the faces in images.detect the faces in images.
recognizer = cv2.face_LBPHFaceRecognizer.create() #face recognition algorithm to recognize front faces (local binary pattern histogram)
...
Face_Images = os.path.join(os.getcwd(), "Face_Images") #Tell the program where we have saved the face images(current working directory) 
print (Face_Images)
